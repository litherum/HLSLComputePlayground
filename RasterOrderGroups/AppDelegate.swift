...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        var vertexData = Array(repeating: SIMD2<Float>(), count: 4 * 100)
        for i in 0 ..< 100 {
            vertexData[4 * i + 0] = SIMD2<Float>(-1, -1)
            vertexData[4 * i + 1] = SIMD2<Float>(-1, 1)
            vertexData[4 * i + 2] = SIMD2<Float>(1, -1)
            vertexData[4 * i + 3] = SIMD2<Float>(1, 1)
        }
        ...
        renderEncoder.drawPrimitives(type: .triangleStrip, vertexStart: 0, vertexCount: 4 * 100)
